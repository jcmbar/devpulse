...
import { Inbox, Loader2, Search, TriangleAlert, X } from "lucide-react";
    ...
    <div className="fixed inset-0 z-50 flex justify-end" role="presentation">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 sm:px-5">
          ...
          {audit ? (
            <div className="mb-4 space-y-3 rounded-md border border-border bg-muted/30 px-4 py-3 text-sm">
              <dl className="grid gap-2 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2">
                  ...
                  <dd>
                    {audit.importId ? (
                      <>
                        <span className="font-mono text-xs">{audit.importId}</span>
                        {audit.batchLabel ? (
          ...
          {audit ? (
            <section className="mb-4 space-y-3 rounded-md border border-border px-4 py-3">
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  <h3 className="text-sm font-semibold tracking-tight">
                    Inspeção de divergência (chaves)
                  </h3>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {compareActive ? (
                  <label className="ml-auto inline-flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    />
                    Mostrar só chaves marcadas na lista
                  </label>
                ) : null}
              </div>
              ...
              {compareActive && compareResults.length > 0 ? (
                <div className="space-y-2 rounded-md border border-border bg-muted/20 px-3 py-2 text-xs">
                  ...
                  <p className="text-muted-foreground">
                    Só DevPulse: {compareSummary.onlyDevPulsePresent} presentes /{" "}
                    {compareSummary.onlyDevPulseAbsent} ausentes · Só Jira:{" "}
                    {compareSummary.onlyJiraPresent} presentes (inesperado) /{" "}
                    {compareSummary.onlyJiraAbsent} ausentes (esperado)
                  </p>
                  ...
                  {absentResults.length > 0 ? (
                    <div className="flex flex-wrap items-center gap-2 border-t border-border pt-2">
                      ...
                      >
                        ...
                        Investigar ausentes no lote (sem filtro de período)
                      </button>
                      <span className="text-muted-foreground">
                        Útil p/ “só Jira” e chaves que sumiram do recorte.
                      </span>
                    </div>
                  ) : null}
                </div>
          ...
          {audit && suspiciousCount > 0 ? (
            <div className="mb-3 rounded-md border border-amber-500/40 bg-amber-500/10 px-4 py-3 text-sm text-amber-950 dark:text-amber-100">
              <div className="flex items-start gap-2">
                <TriangleAlert className="mt-0.5 size-4 shrink-0" strokeWidth={1.9} />
                <div className="space-y-1">
            ...
            cards.length === 0 ? (
              ...
              <div className="space-y-3">
                <p className="text-sm text-muted-foreground">
                  ...
                  {visibleCards.length === 1 ? "" : "s"} · mesma query do ranking.
                </p>
                <DataTable minWidthClassName="min-w-[980px]">
                  <thead>
                    <tr>
                      <th>jira_key</th>
                      <th>due_on</th>
                      <th>unit_test_delivery_on</th>
                      <th>delay_days</th>
                      <th>Classificação</th>
                      {metric === "delayed" ? <th>Justificativa</th> : null}
                      <th>Lote</th>
                    </tr>
                  </thead>
                  <tbody>
...
function AuditCardRow({
  ...
  const divergenceClass =
    compare?.bucket === "only_devpulse"
      ? "bg-rose-500/12 ring-1 ring-inset ring-rose-500/35"
  ...
  return (
    <tr
      className={cn(
        ...
        !divergenceClass &&
          card.isSuspicious &&
          (hasWarning ? "bg-amber-500/10" : "bg-sky-500/5"),
      )}
    >
      <td className="align-top whitespace-nowrap font-medium">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-mono text-sm">{card.jiraKey}</span>
          </div>
          {compare ? (
            <span
              className={cn(
                "w-fit rounded border px-1.5 py-0.5 text-[10px] font-semibold tracking-wide",
                compare.bucket === "only_devpulse"
                  ? "border-rose-500/40 bg-rose-500/15 text-rose-950 dark:text-rose-100"
                  : "border-violet-500/40 bg-violet-500/15 text-violet-950 dark:text-violet-100",
              )}
            >
              {BUCKET_LABEL[compare.bucket]} · na auditoria
            </span>
          ) : null}
          {card.summary ? (
            <p className="max-w-[180px] truncate text-xs text-muted-foreground" title={card.summary}>
              {card.summary}
            </p>
          ) : null}
        </div>
      </td>
      <td className="align-top whitespace-nowrap font-mono text-sm">
        {formatIsoOrNull(card.dueOn)}
      </td>
      <td className="align-top">
        ...
        >
          {formatIsoOrNull(card.unitTestDeliveryOn)}
        </p>
      ...
      <td
        className={cn(
          "align-top whitespace-nowrap font-mono text-sm",
          card.delayDays == null
      ...
      {metric === "delayed" ? (
        <td className="align-top">
          <DelayJustificationCell
            card={card}
            onDecided={onJustificationDecided}
          />
        </td>
      ) : null}
      <td className="align-top font-mono text-[11px] leading-snug text-muted-foreground break-all">
        {card.importId}
      </td>
    </tr>
  );
...
function DelayJustificationCell({
  card,
  onDecided,
}: {
  card: GestorCardAuditItem;
  ...
  return (
    <div className="min-w-[200px] space-y-2 text-xs">
      ...
      <p className="text-muted-foreground" title={justification.developerNote}>
        Dev: {justification.developerNote}
      </p>
      {justification.reviewerNote ? (
        <p className="text-muted-foreground" title={justification.reviewerNote}>
          Gestor: {justification.reviewerNote}
        </p>
      ) : null}
      {justification.status === "pending" ? (
        <div className="space-y-2 border-t border-border pt-2">
          <textarea
            ...
            rows={2}
            placeholder="Nota obrigatória do gestor"
            className="w-full resize-y rounded-md border border-border bg-[var(--surface)] px-2 py-1.5 text-xs outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring)]"
          />
          ...
          <div className="flex flex-wrap gap-1.5">
            <button
              ...
              className="rounded-md border border-emerald-500/40 bg-emerald-500/15 px-2 py-1 font-medium text-emerald-950 disabled:opacity-50 dark:text-emerald-100"
            >
              Aceitar
            </button>
            <button
              ...
              className="rounded-md border border-rose-500/40 bg-rose-500/15 px-2 py-1 font-medium text-rose-950 disabled:opacity-50 dark:text-rose-100"
            >
              Rejeitar
            </button>
          </div>
