-- FASE 3A — compatibilidade e auditoria atômica dos fechamentos
-- Move a remoção da constraint legada para antes do hardening final
-- e garante que status + ledger + evento persistam na mesma transação.

alter table public.developer_time_bank_entries
  drop constraint if exists developer_time_bank_entries_closing_unique;
